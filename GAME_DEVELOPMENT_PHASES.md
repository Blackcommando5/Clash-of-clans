...
| 1. Architecture and data | Partial | Shared building catalog, separate state/progression rules, partial UI files, v6 saves and migrations | Stable village-instance IDs, authored level/content data, clock/repository/command interfaces, revisions and command IDs |
...
| 5. Battle simulation | Partial | Cannon/tower fire, three troop types, four immutable authored layouts, HP/damage, grid routing and wall breach, fixed ticks and matching replays | Immutable arbitrary village snapshots, general-layout navigation, traps/target categories, cross-version replay compatibility, server verification |
| 6. Army preparation | Partial | Saved mixed Raider/Archer/Tank roster, Barracks unlock, camps and weighted capacity, free preparation/readiness; typed campaign deployment, validated ground zones and exact-position replay | Research and broader balancing |
| 7. Complete attack loop | Partial | Checkpointed campaign resumption; latest 20 persistent campaign summaries and verified saved replay playback; eight-step resumable guide; four authored campaign missions, committed army, saved results/progress, once-only first-clear rewards; practice scouting/start, surrender, stars/results and replay | Phone validation and broader polish |
...
- [x] Build a melee fighter, ranged attacker, and defense-targeting tank with original prototype models.
- [ ] Define army readiness/replenishment rules as data; do not assume historical Clash training rules.
- [x] Add deployment-zone validation, troop counts, troop selection, and deploy feedback.
...
- [x] Add persistent campaign battle history with outcomes, army composition and actual reward claims (latest 20 summaries).
- [x] Persist campaign replay playback with command validation, exact reconstruction and explicit version expiry.
- [x] Resume compatible checkpointed campaign attacks, with automatic saves, Save & Return and paused retry on save failure.
- [x] Teach building, collection, upgrading, and deployment through resumable tutorial steps (checklist, screen shortcuts and existing battle instructions).
...
**24 September increment:** saved Raider roster, fixed eight-space starter capacity, free instant preparation and readiness UI are implemented with v4 save migration. See [army validation](ArmyPreparationValidation.txt). Barracks and upgradable Army Camps are now implemented and validated in [the facilities increment](ArmyFacilitiesValidation.txt). Phase 6 remains partial. **Campaign increment:** the prepared roster now powers four authored missions with saved results, unlocks and once-only first-clear rewards; [validation](CampaignValidation.txt) includes spending a reward on a village upgrade. **Mixed-army increment:** Archers, weighted housing, troop selection and typed deterministic replay are now implemented; [validation](MixedArmyValidation.txt) includes preservation of pending rewards from v5 saves. **Ground-deployment increment:** a visible southern zone now accepts selected-troop taps, validates positions and records exact coordinates for replay. See [validation](GroundDeploymentValidation.txt). **Tank increment:** level-2 Barracks unlock a four-space armored melee troop; three-type composition is saved and replayed. See [Tank validation](TankValidation.txt). **Authored-layout increment:** four immutable village layouts now drive battle setup, scouting, deployment bounds and replay; Crossfire Pass and Hillfort extend campaign progression with once-only rewards. See [validation](EnemyLayoutValidation.txt). **Onboarding increment:** the eight-step Village Guide recognizes existing evidence, saves milestones and offers pause/resume hints and screen shortcuts through the build?prepare?attack?upgrade loop. See [validation](TutorialValidation.txt). **Battle-history increment:** the latest 20 campaign outcomes and actual reward claims now survive restart, with a paginated menu and v9 save migration. See [validation](BattleHistoryValidation.txt). **Saved-replay increment:** new campaign history entries retain verified deployment recordings and can be watched after restart; incompatible recordings expire without losing summaries. See [validation](SavedReplayValidation.txt). **Battle-recovery increment:** campaign checkpoints now restore live troops, damage, timers and reserves after restart; compatible attacks resume without another army charge or reward claim. See [validation](BattleRecoveryValidation.txt). **25 September separation increment:** new attacks use deterministic soft troop spacing and guarded waypoint movement under combat rules 7. Rules-6 replay/checkpoint behavior is retained and verified against fixed pre-change fixtures; see [validation](CrowdSeparationValidation.txt). Crowds can still overlap; formation control and phone performance remain unfinished. **Next priority:** broader combat polish and an updated mobile build; physical-device validation is deferred at the user's request. Timers and consumable training costs remain a future design decision. In parallel with planning, the current build needs the Phase 0 phone validation gate. Backend providers, hosting spend and production releases remain future decisions.
