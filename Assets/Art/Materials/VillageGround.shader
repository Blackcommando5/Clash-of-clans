    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                float checker = fmod(abs(floor(cell.x) + floor(cell.y)), 2) * 0.015;
                half3 color = lerp(_OuterColor.rgb, _GrassColor.rgb + checker, village);
                color = lerp(color, _GridColor.rgb, grid * village * _GridStrength);
                float border = (1 - smoothstep(0.06, 0.15, abs(edge - _VillageHalfSize)));
                color = lerp(color, half3(0.57, 0.62, 0.28), border * 0.8);
