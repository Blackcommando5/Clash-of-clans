...
public static class MixedArmyValidation
{
    ...
    static void Rules()
    {
        ...
        string json=JsonUtility.ToJson(legacy).Replace("\"version\":9","\"version\":5");
        Check(VillageState.TryDeserialize(json,out var migrated) && migrated.version==VillageState.SaveVersion && migrated.campaignRunId==id && migrated.campaignArmy==8 && migrated.campaignArchers==0 && migrated.campaignOutcome==PracticeOutcome.Victory,"v5 pending victory preserved");
