...
public static class ArmyPreparationValidation
{
    ...
    public static void Run()
    {
        ...
        try
        {
            ...
            {
                string legacy = JsonUtility.ToJson(state).Replace("\"version\":6", "\"version\":" + version).Replace(",\"army\":[]", "");
                Check(VillageState.TryDeserialize(legacy, out var migrated) && migrated.version == 6 && migrated.IsValid() && !migrated.ArmyReady && migrated.gold == state.gold, "Legacy migration " + version);
            }
            string v3 = JsonUtility.ToJson(state).Replace("\"version\":6", "\"version\":3");
            PlayerPrefs.DeleteKey("Kingdoms.Village.pre-v4");
            PlayerPrefs.SetString(VillageSave.Key, v3);
            Check(VillageSave.TryLoad(out var loaded, out _) && loaded.version == 6 && PlayerPrefs.GetString("Kingdoms.Village.pre-v4") == v3, "Original save backed up");
