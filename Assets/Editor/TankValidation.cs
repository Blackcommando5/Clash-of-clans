...
public static class TankValidation
{
    ...
    static void Rules()
    {
        ...
        json=JsonUtility.ToJson(old).Replace("\"version\":9","\"version\":6");PlayerPrefs.DeleteKey("Kingdoms.Village.pre-v7");PlayerPrefs.SetString(VillageSave.Key,json);
        Check(VillageSave.TryLoad(out loaded,out _) && loaded.version==VillageState.SaveVersion && loaded.campaignTanks==0 && loaded.campaignOutcome==PracticeOutcome.Victory && loaded.campaignRunId==old.campaignRunId && PlayerPrefs.GetString("Kingdoms.Village.pre-v7")==json,"v6 pending reward migration and backup");
