...
public static class ResourceMilestoneValidation
{
    ...
    static void StateChecks()
    {
        ...
        Check(VillageState.TryDeserialize(legacy,out var migrated) && migrated.version==VillageState.SaveVersion && migrated.gold==9999 && migrated.elixir==50 && migrated.CollectableGold==321,"Legacy progress preserved");
    ...
    static void ProgressionChecks()
    {
        ...
        Check(VillageState.TryDeserialize(JsonUtility.ToJson(old),out var upgraded) && upgraded.version==VillageState.SaveVersion && upgraded.buildings[0].level==1,"Version 2 save gains valid levels");
    }
    ...
    static void Tick()
    {
        try
        {
            ...
            if(step==2)
            {
                ...
                Check(game.State.version==VillageState.SaveVersion && game.State.IsValid(),"Reloaded save is valid");
