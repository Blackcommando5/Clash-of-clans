...
public static class ResourceMilestoneValidation
{
    ...
    public static void RunRules()
    {
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(Root, "ArmyResourceRegression.txt"),
                "PASS: " + assertions + " existing resource/progression rule assertions with v5 saves. Unity " + Application.unityVersion + ". State-only run; no phone testing.");
    ...
    static void StateChecks()
    {
        ...
        Check(VillageState.TryDeserialize(legacy,out var migrated) && migrated.version==5 && migrated.gold==9999 && migrated.elixir==50 && migrated.CollectableGold==321,"Legacy progress preserved");
    ...
    static void ProgressionChecks()
    {
        ...
        Check(VillageState.TryDeserialize(JsonUtility.ToJson(old),out var upgraded) && upgraded.version==5 && upgraded.buildings[0].level==1,"Version 2 save gains valid levels");
    }
    ...
    static void Tick()
    {
        try
        {
            ...
            if(step==2)
            {
                ...
                Check(game.State.version==5 && game.State.IsValid(),"Reloaded save is valid");
