...
public static class CampaignValidation
{
    ...
    static void Rules()
    {
        ...
        string legacy=JsonUtility.ToJson(s).Replace("\"version\":9","\"version\":4");
        Check(VillageState.TryDeserialize(legacy,out var migrated) && migrated.version==VillageState.SaveVersion && migrated.ArmyHousing==8 && !migrated.HasCampaignRun && migrated.campaignCleared==0,"v4 roster retained");
    ...
    static void Tick()
    {
        ...
        try
        {
            ...
            if(step==3)
            {
                ...
                Finish("PASS: v4-to-v8 roster migration and backup; empty/locked/duplicate start guards; durable whole-roster commitment; interrupted abandonment; recording verification, ticket/layout/army mismatch rejection; full-storage reward preservation; once-only first clear after reload; repeat/loss policy; both missions winnable; 80-unit deterministic replay; malformed saves/budgets; real campaign menu/scout/start/deploy/results/replay; pending-result scene reload and claim; reward-funded Town Hall upgrade; free re-preparation, second mission surrender/dismiss; practice isolation. Unity "+Application.unityVersion+". Original two-mission regression; expanded layouts covered by EnemyLayoutValidation; no server authority, persisted replay, APK or phone validation.",0);
            }
        }
        catch(Exception e){Finish("FAIL: "+e,1);}
    }
