...
public static class TutorialValidation
{
    ...
    static void Rules()
    {
        ...
        s.tutorialMilestones=0;s.tutorialHintsPaused=false;string old=JsonUtility.ToJson(s).Replace("\"version\":8","\"version\":7");PlayerPrefs.DeleteKey("Kingdoms.Village.pre-v8");PlayerPrefs.SetString(VillageSave.Key,old);
        Check(VillageSave.TryLoad(out loaded,out _) && loaded.version==8 && loaded.TutorialStep==2 && !loaded.tutorialHintsPaused && PlayerPrefs.GetString("Kingdoms.Village.pre-v8")==old,"v7 migration infers progress and keeps backup");
        ...
        string ticket=pending.campaignRunId;string prior=JsonUtility.ToJson(pending).Replace("\"version\":8","\"version\":7");
