...
| Scout inspection | Tap enemy buildings before an attack for health and role details; defenses show damage and a center-based attack-range circle; Next Defense cycles targets without spending troops |
...
The existing Android development APK builds and passes package checks, but predates battle history, saved replays, battle recovery, replay controls, army reuse, wall-row placement, troop separation, wall upgrades, connected-wall movement and scout inspection. Rebuild it to include these features. Physical-phone testing remains pending and has been deferred by the user. See [Android validation](ANDROID_BUILD_VALIDATION.md).
...
5. Tap Attack! to scout the practice battlefield. Tap an enemy building or use Next Defense to inspect its health, damage and range circle; Clear Inspection restores the overview. Choose a challenge, then press Start Attack. Deploy Left, Center or Right spends one of eight practice raiders per tap; tapping the green ground deployment zone also deploys a troop.
...
### Enemy-building inspection while scouting - 25 September 2026

Before pressing **Start Attack**, tap an enemy building to inspect it. Its footprint gains a gold outline, and the scouting panel shows its health and role. Defenses also show damage per second and a gold attack-range circle. **Next Defense** cycles through every defense in the authored layout, then returns to the first, making small targets easier to inspect. **Clear Inspection**, or tapping empty ground, removes the outlines and restores the mission overview.

**Beginner walkthrough:** Open **Attack!**, press **Next Defense** to inspect the Cannon, and compare its nine-cell range with the Archer Tower's ten-cell range on the next press. Tap the Town Hall to see its health and star objective; it has no attack circle. Tap a wall to see its movement-blocking role and the reminder that walls do not count toward destruction. In **Prepare Army > Campaign**, scout Crossfire Pass and cycle through all three defenses before choosing where to deploy. Hillfort's storage can also be inspected. Campaign reward status remains visible while a building is selected.

The circle uses the selected enemy entity's actual attack range, measured from its center, matching the combat distance check. It indicates coverage, not which troop the defense will choose. These are authored enemy stats rather than the levels of your home buildings. The timer remains at zero during scouting, and inspection does not deploy troops, spend your prepared army, award resources, create a campaign run or write a village save. Pressing Start Attack still commits the prepared campaign army through the existing save checks; a failed save keeps you in scouting and shows the error.

Selection is temporary. Starting combat, switching practice challenges, restarting a practice encounter or returning home clears it. Inspection controls and outlines are unavailable during live attacks, resumed attacks and replays. Taps over UI, drags, canceled gestures and invalid screen coordinates cannot select buildings. Application pause and focus loss cancel a held scouting gesture. Scouting keeps the existing fixed battle camera.

Study [selection, input and range rendering](Assets/Scripts/UI/VillageBattleScouting.cs), [battle lifecycle integration](Assets/Scripts/UI/VillagePracticeBattle.cs), [authored enemy layouts](Assets/Scripts/Core/EnemyLayout.cs), and [validation source](Assets/Editor/BattleScoutingValidation.cs). Saves remain **version 9**, new combat remains **rules 7**, and rules-6 replay/checkpoint compatibility is unchanged.

Validation: [BattleScoutingValidation.txt](BattleScoutingValidation.txt). Previews: [Cannon range](BattleScoutingPreviews/scout-cannon-range.png), [16:9 range](BattleScoutingPreviews/scout-cannon-range-16x9.png), [Crossfire Archer Tower](BattleScoutingPreviews/scout-crossfire-range.png), and [Hillfort storage](BattleScoutingPreviews/scout-storage-details.png). Isolated Unity 6000.3.13f1 checks passed for model-collider selection, every point on the range circle, selected-entity health/damage, building roles, defense cycling, clear/empty-ground behavior, unchanged battle hash/timer/army/economy/save, both practice challenges and all four campaign layouts. Gesture checks covered UI-origin taps, drags, invalid coordinates, pause and focus loss. Attack/replay/home cleanup, normal deployment, campaign commitment, failed-save error visibility and resumed-attack guards also passed, alongside existing deployment, recovery, replay, history, campaign and progression rule regressions. Both landscape layouts were inspected. Gestures were exercised through their handlers; physical multitouch and real storage failures remain untested.

Known limits: one inspected building at a time; no combined coverage map or live-battle inspection. A range circle may extend beyond the camera view. Physical multitouch, phone layout/performance and an updated APK remain untested; phone testing stays deferred.
